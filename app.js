...
$("#submitBtn").onclick=()=>{   const nama=$("#nama").value.trim();   const kelas=$("#kelas").value;   const jantina=$("#jantina").value;    const requestId =     "REQ-" +     Date.now() +     "-" +     Math.random()       .toString(36)       .slice(2,10);    const payload={     action:"submit",     requestId,     nama,     kelas,     jantina,     answers:[...answers]   };    if(!isValidStudentPayload(payload)){      if(answers.some(v=>!v)){       return alert(         "Masih ada item yang belum dijawab."       );     }      return alert(       "Maklumat murid tidak lengkap atau tidak sah."     );   }    const endpoint=     window.ITP_CONFIG &&     window.ITP_CONFIG.SUBMIT_ENDPOINT;    if(!endpoint){     return alert(       "Konfigurasi penghantaran belum tersedia."     );   }    const btn=$("#submitBtn");   const oldText=btn.textContent;    btn.disabled=true;   btn.textContent="SEDANG MENGHANTAR...";    let settled=false;    function cleanup(){     window.removeEventListener(       "message",       handleSubmitResult     );   }    function resetForm(){     answers=Array(150).fill(null);     page=0;      $("#nama").value="";     $("#kelas").value="";     $("#jantina").value="";      renderPage();     updateProgress();   }    function restoreButton(){     btn.disabled=false;     btn.textContent=oldText;   }    function handleSubmitResult(event){      if(       event.origin !==       "https://script.google.com" &&       event.origin !==       "https://script.googleusercontent.com"     ){       return;     }      const data=event.data;      if(       !data ||       data.type!=="ITP_SUBMIT_RESULT"     ){       return;     }      if(       data.requestId !== requestId     ){       return;     }      settled=true;     cleanup();     restoreButton();      if(data.success===true){        alert(         "Jawapan anda telah berjaya direkodkan. Terima kasih kerana menjawab dengan jujur."       );        resetForm();       return;     }      if(       data.code ===       "DUPLICATE_RECORD"     ){        alert(         "Rekod anda telah diterima sebelum ini. Sila rujuk guru jika perlu pembetulan."       );        return;     }      alert(       data.message ||       "Penghantaran tidak berjaya. Sila cuba semula."     );   }    window.addEventListener(     "message",     handleSubmitResult   );    try{      const form=       document.createElement("form");      form.method="POST";     form.action=endpoint;     form.target="itpSubmitFrame";     form.style.display="none";      const input=       document.createElement("input");      input.type="hidden";     input.name="payload";     input.value=       JSON.stringify(payload);      form.appendChild(input);     document.body.appendChild(form);      form.submit();      setTimeout(()=>{       form.remove();     },1500);      setTimeout(()=>{        if(settled){         return;       }        cleanup();       restoreButton();        alert(         "Sistem belum menerima pengesahan penghantaran. Sila semak sambungan dan cuba semula."       );      },15000);    }catch(err){      console.error(err);      cleanup();     restoreButton();      alert(       "Penghantaran tidak berjaya. Sila cuba semula."     );   } };
